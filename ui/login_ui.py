...
class LoginFrame(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        card = ctk.CTkFrame(
            ...
            width=880, height=560, border_width=1,
        ...
        right = ctk.CTkFrame(
            card, fg_color=THEME["sidebar"], width=380, corner_radius=24,
        )
        right.pack(side="right", fill="both", expand=True, padx=(0, 8), pady=8)
        right.pack_propagate(False)

        canvas = tk.Canvas(right, bg=THEME["sidebar"], highlightthickness=0, bd=0)
        ...
        self.password_entry.pack(side="left", fill="x", expand=True, padx=(0, 4), pady=6)

        self._eye_btn = ctk.CTkButton(
            pass_frame, text="👁", width=36, height=36, corner_radius=14,
            fg_color="transparent", hover_color=MODERN_THEME["surface_hover"],
            text_color=THEME["text_sub"],
            font=(MODERN_THEME["font_family"], 14),
    ...
    def _draw_right(self, canvas, width, height):
        ...
        canvas.create_oval(width * 0.46, height * -0.10,
            width * 1.20, height * 0.50,
            fill=THEME["sidebar_alt"], outline="")
        canvas.create_oval(width * 0.52, height * 0.42,
            width * 1.20, height * 1.18,
            fill=THEME["primary_dark"], outline="")
        canvas.create_rectangle(42, 42, width - 42, 44,
            fill=THEME["primary"], outline="")
        canvas.create_text(42, 86, anchor="w", text="CHURCHTRACK",
            font=(MODERN_THEME["font_family"], 11, "bold"),
            fill=THEME["primary_soft"])
        canvas.create_text(42, 146, anchor="w",
            text="Focused tools for\nfaithful financial\nstewardship.",
            font=(MODERN_THEME["font_family"], 26, "bold"),
            fill=THEME["bg_card"])
        canvas.create_text(42, 292, anchor="w",
            text="Donations, expenses, events,\nreports — all in one place.",
            font=(MODERN_THEME["font_family"], 12),
            fill=THEME["border_strong"])
        canvas.create_rectangle(42, height - 100, width - 42, height - 44,
            fill=THEME["text_main"], outline=THEME["border_strong"], width=1)
        canvas.create_text(62, height - 80, anchor="w",
            text="ChurchTrack v2",
            font=(MODERN_THEME["font_family"], 12, "bold"),
            fill=THEME["bg_card"])
        canvas.create_text(62, height - 60, anchor="w",
            text="Local parish management system.",
            font=(MODERN_THEME["font_family"], 11),
